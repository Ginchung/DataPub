...
|模型|回复|随机|
|-|-|-|
|ChatGPT<br>(text-davinci-003，下同)|还是有一定的难度. 同时, transformer在NLP领域的应用也是有一定的局限性的. 比如说, transformer在解决一些短文本的任务上, 比如文本分类, 情感分析等, 其表现还是不如RNN. 另外, transformer在解决一些长文本的任务上, 比如机器翻译, 摘要等, 其表现也不是很理想. 因此, transformer在不同的任务上的表现也是有很大差异的.|无|
